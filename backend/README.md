...
```bash
incident-scribe-backend$ sam build
```
...
```bash
incident-scribe-backend$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
incident-scribe-backend$ sam local start-api
incident-scribe-backend$ curl http://localhost:3000/
```
...
```bash
incident-scribe-backend$ sam logs -n HelloWorldFunction --stack-name incident-scribe-backend --tail
```
...
```bash
incident-scribe-backend$ cd hello-world
...
```bash
sam delete --stack-name incident-scribe-backend
```
